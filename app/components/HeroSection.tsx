...
// Animated Scientist/Character Component - Side View
function ScientistCharacter() {
  return (
    <div className="relative w-full h-full">
      {/* Character sitting at desk - side view (in front) */}
      <motion.div
      ...
      >
        ...
        >
          {/* Head shape */}
          <div className="w-20 h-24 bg-gradient-to-br from-gray-700 to-gray-900 rounded-full border-2 border-gray-600 relative">
            {/* Hair */}
            <div className="absolute -top-3 -left-2 w-24 h-16 bg-gradient-to-br from-gray-800 to-gray-900 rounded-t-full" />
            
            {/* Ear */}
            <div className="absolute left-0 top-10 w-4 h-6 bg-gray-700 rounded-full border-l-2 border-gray-600" />
            
            {/* Glasses */}
            <div className="absolute right-4 top-8 w-8 h-8 border-2 border-blue-400/50 rounded-full bg-blue-500/10">
              {/* Lens reflection */}
              <motion.div
            ...
            {/* Eye visible through glasses */}
            <motion.div
              animate={{ scaleY: [1, 0.1, 1] }}
              transition={{ duration: 4, repeat: Infinity, repeatDelay: 3 }}
              className="absolute right-6 top-10 w-2 h-2 bg-cyan-400 rounded-full shadow-lg shadow-cyan-400/50"
            />
            
            {/* Nose */}
            <div className="absolute right-2 top-12 w-3 h-4 bg-gray-600 rounded-bl-full" />
          </div>

          {/* Neck */}
          <div className="absolute -bottom-6 left-6 w-10 h-8 bg-gradient-to-b from-gray-700 to-gray-800" />
        </motion.div>

        {/* Upper body/torso - sitting position */}
        <div className="absolute top-16 left-0">
          {/* Shoulders and chest */}
          <div className="w-28 h-32 bg-gradient-to-br from-gray-800 to-gray-900 rounded-lg border-2 border-gray-700 relative">
            {/* Hoodie/shirt detail */}
            <div className="absolute inset-2 bg-gradient-to-br from-blue-900/20 to-purple-900/10 rounded-lg" />
            
            {/* Collar */}
            <div className="absolute top-0 left-1/3 w-12 h-6 bg-gray-700 rounded-b-lg" />
          </div>

          {/* Right arm extending forward to keyboard */}
          <div className="absolute top-20 left-full">
            {/* Upper arm */}
            <motion.div
              animate={{ rotate: [20, 23, 20] }}
              transition={{ duration: 0.8, repeat: Infinity }}
              className="w-16 h-5 bg-gradient-to-r from-gray-800 to-gray-700 rounded-full origin-left shadow-lg"
              style={{ transformOrigin: 'left center' }}
            />
            
            {/* Forearm extending to keyboard */}
            <motion.div
              animate={{ rotate: [-10, -12, -10] }}
              transition={{ duration: 0.6, repeat: Infinity }}
              className="absolute top-0.5 left-14 w-20 h-4 bg-gradient-to-r from-gray-700 to-gray-600 rounded-full origin-left"
              style={{ transformOrigin: 'left center', transform: 'rotate(-10deg)' }}
            >
              {/* Hand on keyboard with fingers */}
              <motion.div
                animate={{ scaleY: [1, 0.95, 1] }}
                transition={{ duration: 0.6, repeat: Infinity }}
                className="absolute right-0 -bottom-0.5 w-10 h-6 bg-gray-600 rounded-lg shadow-lg"
              >
                {/* Fingers */}
                <div className="absolute -bottom-1 left-1 w-1.5 h-3 bg-gray-600 rounded-b" />
                <div className="absolute -bottom-1 left-3 w-1.5 h-3 bg-gray-600 rounded-b" />
                <div className="absolute -bottom-1 right-3 w-1.5 h-3 bg-gray-600 rounded-b" />
                <div className="absolute -bottom-1 right-1 w-1.5 h-3 bg-gray-600 rounded-b" />
              </motion.div>
            </motion.div>
          </div>

          {/* Left arm on armrest */}
          <div className="absolute top-24 -left-6 w-20 h-4 bg-gradient-to-r from-gray-800 to-gray-700 rounded-full" style={{ transform: 'rotate(-5deg)' }}>
            <div className="absolute right-0 -bottom-1 w-6 h-4 bg-gray-700 rounded-lg" />
          </div>
        </div>

        {/* Lower body - sitting on chair */}
        <div className="absolute top-40 left-2">
          {/* Thighs */}
          <div className="w-26 h-14 bg-gradient-to-b from-gray-800 to-gray-900 rounded-lg shadow-lg" />
          
          {/* Shins */}
          <div className="absolute -bottom-14 left-3 w-5 h-18 bg-gradient-to-b from-gray-800 to-gray-700 rounded-lg shadow-md" />
          <div className="absolute -bottom-14 right-4 w-5 h-18 bg-gradient-to-b from-gray-800 to-gray-700 rounded-lg shadow-md" />
          
          {/* Feet with shoe detail */}
          <div className="absolute -bottom-14 left-3 w-12 h-5 bg-gradient-to-r from-gray-700 to-gray-800 rounded-lg shadow-lg" />
          <div className="absolute -bottom-14 right-4 w-12 h-5 bg-gradient-to-r from-gray-700 to-gray-800 rounded-lg shadow-lg" />
        </div>
      </motion.div>

      {/* Office chair - side view (positioned behind character) */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        {/* Chair back */}
        <div className="w-16 h-40 bg-gradient-to-b from-gray-800 to-gray-900 rounded-2xl border-2 border-gray-700 opacity-60" />
        
        {/* Armrest */}
        <div className="absolute top-20 right-2 w-12 h-3 bg-gray-700 rounded-full opacity-60" />
        
        {/* Seat (partially visible behind character) */}
        <div className="absolute top-32 left-0 w-24 h-8 bg-gradient-to-r from-gray-700 to-gray-800 rounded-lg border-2 border-gray-600 opacity-50" />
        
        {/* Chair leg/base */}
        <div className="absolute top-40 left-8 w-3 h-12 bg-gray-700" />
        <div className="absolute top-52 left-2 w-16 h-3 bg-gray-800 rounded-full" />
        
        {/* Wheels */}
        <div className="absolute top-52 left-0 w-4 h-4 bg-gray-900 rounded-full border border-gray-600" />
        <div className="absolute top-52 right-2 w-4 h-4 bg-gray-900 rounded-full border border-gray-600" />
      </motion.div>
    </div>
  );
}
